...
const { errorMessage, loginMutation, isPending } = useAuthActions()
  ...
  <div class="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-12">
    ...
    <section class="relative w-full max-w-md rounded-lg border border-border/60 bg-card/84 p-8 shadow-2xl backdrop-blur-xl">
      <div class="mb-8 flex items-center gap-4">
        ...
        <div class="flex-1">
          <h1 class="text-xl md:text-2xl text-primary font-bold leading-tight">Login to Thura Note</h1>
        </div>
      </div>
      ...
      <form class="space-y-5" @submit.prevent="onSubmit">
        ...
        <Button class="h-12 w-full rounded-lg  text-base font-semibold" type="submit" :disabled="isPending">
          {{ isPending ? 'Logging in...' : 'Login' }}
        </Button>

        <p class="text-center text-sm text-muted-foreground">
          New here?
          <RouterLink :to="{ name: 'register' }" class="font-semibold text-primary hover:underline">Create account</RouterLink>
        </p>
      </form>
    </section>
  </div>
